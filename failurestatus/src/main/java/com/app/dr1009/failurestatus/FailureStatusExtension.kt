...
fun androidx.fragment.app.Fragment.networkErrorBar(
    anchor: View,
    status: FailureStatus,
    message: (throwable: Throwable?) -> String,
    ...
    val bar = com.google.android.material.snackbar.Snackbar.make(anchor, status.createMessage(message), duration)
    if (isShowAction) {
        bar.setAction(actionMessage(status.throwable)) { action(status.throwable) }
    }
